...
static int doca_gw_dpdk_modify_eth_item(struct doca_dpdk_item_entry *entry,
...
static void doca_gw_dpdk_build_eth_flow_item(struct doca_dpdk_item_entry *entry,
					     struct doca_gw_match *match)
{
...
static int doca_gw_dpdk_modify_vlan_item(struct doca_dpdk_item_entry *entry,
					 struct doca_gw_match *match)
{
...
static int doca_gw_dpdk_modify_ipv4_item(struct doca_dpdk_item_entry *entry,
					 struct doca_gw_match *match)
{
...
static void
doca_gw_dpdk_build_ipv4_flow_item(struct doca_dpdk_item_entry *entry,
 ...
 */
static int
doca_gw_dpdk_modify_ipv6_item(__rte_unused struct doca_dpdk_item_entry *entry,
			      __rte_unused struct doca_gw_match *match)
{
	return 0;
}
...
static void
doca_gw_dpdk_build_ipv6_flow_item(struct doca_dpdk_item_entry *entry,
				  struct doca_gw_match *match, uint8_t type)
{
...
static int doca_gw_dpdk_modify_vxlan_item(struct doca_dpdk_item_entry *entry,
					  struct doca_gw_match *match)
{
...
static void
doca_gw_dpdk_build_vxlan_flow_item(struct doca_dpdk_item_entry *entry,
				   struct doca_gw_match *match)
{
	struct rte_flow_item *flow_item = entry->item;
	struct rte_flow_item_vxlan *spec = &entry->item_data.vxlan.spec;
...
static int doca_gw_dpdk_modify_gre_key_item(struct doca_dpdk_item_entry *entry,
					    struct doca_gw_match *match)
{
...
static void
doca_gw_dpdk_build_gre_key_flow_item(struct doca_dpdk_item_entry *entry,
				     struct doca_gw_match *match)
{
	struct rte_flow_item *flow_item = entry->item;
	uint32_t *spec = &entry->item_data.gre_key.spec;
...
static void
doca_gw_dpdk_build_inner_eth_flow_item(struct doca_dpdk_item_entry *entry,
				       struct doca_gw_match *match)
{
	struct rte_flow_item *flow_item = entry->item;
	struct rte_flow_item_eth *spec = &entry->item_data.eth.spec;
...
static int doca_gw_dpdk_modify_tcp_flow_item(struct doca_dpdk_item_entry *entry,
					     struct doca_gw_match *match)
{
...
static int doca_gw_dpdk_modify_udp_flow_item(struct doca_dpdk_item_entry *entry,
					     struct doca_gw_match *match)
{
...
#define NEXT_ITEM (&pipe_flow->item_entry[idx++])
	...
	if (match->tun.type != DOCA_TUN_NONE) {
		...
		case DOCA_TUN_VXLAN:
			...
			doca_gw_dpdk_build_udp_flow_item(NEXT_ITEM, match,
							 type);
			doca_gw_dpdk_build_vxlan_flow_item(NEXT_ITEM, match);
			doca_gw_dpdk_build_inner_eth_flow_item(NEXT_ITEM,
							       match);
			break;
		case DOCA_TUN_GRE:
		...
		type = INNER_MATCH;
		if (doca_match_is_ipv4(match, type))
			doca_gw_dpdk_build_ipv4_flow_item(NEXT_ITEM, match,
							  type);
		else
			doca_gw_dpdk_build_ipv6_flow_item(NEXT_ITEM, match,
							  type);
	}
 ...
 * @return
 */
static void doca_gw_dpdk_build_ether_header(uint8_t **header,
...
static void doca_gw_dpdk_build_udp_header(uint8_t **header,
					  struct doca_gw_pipeline_cfg *cfg)
{
...
static void doca_gw_dpdk_build_vxlan_header(uint8_t **header,
					    struct doca_gw_pipeline_cfg *cfg)
{
...
static void doca_gw_dpdk_build_gre_header(uint8_t **header,
					  struct doca_gw_pipeline_cfg *cfg)
{
	uint32_t *key_data;
...
static void doca_gw_dpdk_build_endecap_data(uint8_t **header,
					    struct doca_gw_pipeline_cfg *cfg,
					    uint16_t flags)
{
	uint8_t idx;
...
static void
doca_gw_dpdk_build_decap_action(struct doca_dpdk_action_entry *entry,
				struct doca_gw_pipeline_cfg *cfg,
				uint8_t decap_layer)
{
	uint8_t *header;
...
static int
doca_gw_dpdk_build_tunnel_action(struct doca_dpdk_action_entry *entry,
				 struct doca_gw_pipeline_cfg *cfg)
{
	uint8_t layer;
	struct doca_gw_match *match = cfg->match;
...
static int
doca_gw_dpdk_modify_ipv4_addr_action(struct doca_dpdk_action_entry *entry,
				     struct doca_gw_actions *pkt_action)
{
...
static void
doca_gw_dpdk_build_ipv4_addr_action(struct doca_dpdk_action_entry *entry,
				    struct doca_gw_pipeline_cfg *cfg,
				    uint8_t type)
{
	struct rte_flow_action *action = entry->action;
...
static int
doca_gw_dpdk_modify_l4_port_action(struct doca_dpdk_action_entry *entry,
				   struct doca_gw_actions *pkt_action)
{
	uint16_t l4port;
	struct rte_flow_action *action = entry->action;
...
static void
doca_gw_dpdk_build_l4_port_action(struct doca_dpdk_action_entry *entry,
				  struct doca_gw_pipeline_cfg *cfg,
				  uint8_t type)
{
	uint16_t l4port;
...
static void
doca_gw_dpdk_build_dec_ttl_action(struct doca_dpdk_action_entry *entry)
{
	struct rte_flow_action *action = entry->action;
...
static inline uint64_t doca_gw_get_rss_type(uint32_t rss_type)
{
...
static int doca_gw_dpdk_build_rss_action(struct doca_dpdk_action_entry *entry,
...
static int doca_gw_dpdk_create_meter_profile(uint16_t port_id, uint32_t id,
...
static int doca_gw_dpdk_create_meter_policy(uint16_t port_id,
					    uint32_t policy_id,
					    struct doca_fwd_table_cfg *fwd_cfg)
{
	int ret;
...
static int doca_gw_dpdk_build_meter_action(
			struct doca_gw_pipelne_entry *pipe_entry,
			uint16_t port_id,
				  ...
				  struct doca_fwd_table_cfg *cfg)
{
	...
	if (mon->flags & DOCA_GW_METER) {
		...
		if (doca_gw_dpdk_build_meter_action(pipe_entry, port_id, entry,
						    mon, cfg))
			return -1;
	}
	return 0;
...
static int doca_gw_dpdk_modify_pipe_match(struct doca_gw_pipe_dpdk_flow *pipe,
					  struct doca_gw_match *match)
{
	int idex, ret;
...
static int doca_gw_dpdk_modify_pipe_actions(struct doca_gw_pipe_dpdk_flow *pipe,
...
doca_gw_dpdk_create_flow(uint16_t port_id, const struct rte_flow_attr *attr,
...
static struct rte_flow *doca_gw_dpdk_create_root_jump(uint16_t port_id)
{
...
static struct rte_flow *doca_gw_dpdk_create_def_rss(uint16_t port_id)
{
	struct rte_flow_attr attr;
	struct rte_flow_item items[MAX_ITEMS];
	struct rte_flow_action actions[MAX_ACTIONS];
...
static struct rte_flow *doca_gw_dpdk_pipe_create_entry_flow(
					...
					__rte_unused struct doca_gw_error *err)
{
	...
	if (!(mon->flags & DOCA_GW_METER) &&
	    doca_gw_dpdk_build_fwd(pipe, cfg)) {
		DOCA_LOG_ERR("build pipe fwd action fail.");
		return NULL;
	}
	doca_gw_dpdk_build_end_action(pipe);
	return doca_gw_dpdk_create_flow(pipe->port_id, &pipe->attr, pipe->items,
					pipe->actions);
}

struct doca_gw_pipelne_entry *doca_gw_dpdk_pipe_create_flow(
	struct doca_gw_pipeline *pipeline, struct doca_gw_match *match,
	struct doca_gw_actions *actions, struct doca_gw_monitor *mon,
	struct doca_fwd_table_cfg *cfg, struct doca_gw_error *err)
{

	struct doca_gw_pipelne_entry *entry;

	entry = (struct doca_gw_pipelne_entry *)malloc(
	    sizeof(struct doca_gw_pipelne_entry));
	if (entry == NULL)
		return NULL;
	entry->pipe_entry = doca_gw_dpdk_pipe_create_entry_flow(
	    entry, &pipeline->flow, match, actions, mon, cfg, err);
...
int doca_gw_dpdk_pipe_free_entry(uint16_t portid,
				 struct doca_gw_pipelne_entry *entry)
{
	int ret;
	...
	if (entry->meter_id) {
		...
		if (ret) {
			DOCA_LOG_ERR("Port %u free meter rule id %u err, type "
				     "%d message: %s",
				     portid, entry->meter_id, mtr_err.type,
				     mtr_err.message ? mtr_err.message
						     : "(no stated reason)");
			return -1;
		}
	}
	...
	if (entry->meter_policy_id) {
		ret = rte_mtr_meter_policy_delete(
		    portid, entry->meter_policy_id, &mtr_err);
		if (ret) {
			DOCA_LOG_ERR("Port %u free meter policy id %u err, "
				     "type %d message: %s",
				     portid, entry->meter_policy_id,
				     mtr_err.type,
				     mtr_err.message ? mtr_err.message
						     : "(no stated reason)");
			return -1;
	...
	if (entry->meter_profile_id) {
		ret = rte_mtr_meter_profile_delete(
		    portid, entry->meter_profile_id, &mtr_err);
		if (ret) {
			DOCA_LOG_ERR("Port %u free meter profile id %u err, "
				     "type %d message: %s",
				     portid, entry->meter_profile_id,
				     mtr_err.type,
				     mtr_err.message ? mtr_err.message
						     : "(no stated reason)");
			return -1;
 ...
 * @return
 */
static int doca_gw_dpdk_create_pipe_flow(struct doca_gw_pipe_dpdk_flow *flow,
					 struct doca_gw_pipeline_cfg *cfg,
					 struct doca_gw_error *err)
{
	int ret;
...
doca_gw_dpdk_create_pipe(struct doca_gw_pipeline_cfg *cfg,
			 struct doca_gw_error *err)
{
	int ret;
	uint32_t idx;
...
static struct doca_gw_port *doca_alloc_port_byid(uint8_t port_id,
						 struct doca_gw_port_cfg *cfg)
{
	struct doca_gw_port *port;

	port = (struct doca_gw_port *)malloc(sizeof(struct doca_gw_port) +
					     cfg->priv_data_size);
	if (port == NULL)
		return NULL;
...
struct doca_gw_port *
doca_gw_dpdk_port_start(struct doca_gw_port_cfg *cfg,
			__rte_unused struct doca_gw_error *err)
{
