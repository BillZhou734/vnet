...
static struct doca_fwd_tbl *gw_build_port_fwd(int port_id)
{
...
static void gw_build_tun_match(struct doca_gw_match *match)
{
	switch (gw_tun_type) {
	case DOCA_TUN_VXLAN:
...
static struct doca_gw_pipeline *gw_build_ul_ol(struct doca_gw_port *port)
{
	...
	struct doca_gw_monitor monitor = {.m.cbs = UINT64_MAX,
					  .m.cir = UINT64_MAX};
			       ...
			       struct doca_gw_pipeline *pipeline)
{
	struct doca_gw_match match;
	struct doca_gw_actions actions = {0};
	struct doca_gw_monitor monitor = {.m.cbs = GW_DEFAULT_CBS,
	...
	return doca_gw_pipeline_add_entry(
