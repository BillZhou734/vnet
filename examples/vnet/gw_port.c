...
int gw_init_port(int port_id,int nr_queues)
{
	...
	struct rte_eth_dev_info dev_info;
        ...
        if (mbuf_pool == NULL) {
            mbuf_pool = rte_pktmbuf_pool_create("mbuf_pool", 4096 * 32, 128, 0,
	...
	DOCA_LOG_INFO("initializing port: %d,nr_queues:%d,nb_desc:%d\n", port_id, nr_queues,nb_desc);
	...
	for (i = 0; i < nr_queues; i++) {
		ret = rte_eth_rx_queue_setup(port_id, i, nb_desc,
	...
	for (i = 0; i < nr_queues; i++) {
		ret = rte_eth_tx_queue_setup(port_id, i, nb_desc,
				rte_eth_dev_socket_id(port_id),
				&txq_conf);
		if (ret < 0) {
			rte_exit(EXIT_FAILURE,
...
extern struct gw_cpu_usage gw_cpu[];
static void
gw_port_stats_display(uint16_t port_id)
{
	...
	for (i = 0; i < dev_info.nb_rx_queues; i++) {
		printf("  Stats reg %2d RX-packets: %-10"PRIu64
		       ...
		       (float)(rte_atomic64_read(&gw_cpu[i].total_rx_time) * 100)
		       	/ (current - rte_atomic64_read(&gw_cpu[i].start)));
	}
