...
struct doca_gw_cfg {
	...
	uint16_t queues;
	/* each offload thread should use a different queue id */
	bool aging;
 ...
 */
struct doca_gw_port *doca_gw_port_start(struct doca_gw_port_cfg *cfg,
 ...
 */
struct doca_gw_pipelne_entry *doca_gw_pipeline_add_entry(
	uint16_t pipe_queue, struct doca_gw_pipeline *pipeline,
	struct doca_gw_match *match, struct doca_gw_actions *actions,
	struct doca_gw_monitor *mon, struct doca_fwd_tbl *fwd,
	struct doca_gw_error *err);
 ...
 */
int doca_gw_pipeline_update_default(uint16_t pipe_queue,
				    struct doca_gw_port *port,
				    struct doca_fwd_tbl *fwd,
				    struct doca_gw_error *err);
 ...
 */
bool doca_gw_query_aging(struct doca_gw_pipelne_entry *arr, int arr_len,
			 int *n);
