...
struct endecap_layer {
	uint16_t layer;
	void (*fill_data)(uint8_t **header, struct doca_gw_pipeline_cfg *cfg);
};
...
static inline bool doca_match_is_ipv4(struct doca_gw_match *match, uint8_t type)
{
...
static inline rte_be16_t doca_gw_get_l3_protol(struct doca_gw_match *match,
					       uint8_t type)
{
	uint16_t protocol;
